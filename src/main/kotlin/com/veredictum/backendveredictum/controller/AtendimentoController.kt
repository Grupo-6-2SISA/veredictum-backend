    ...
    class AtendimentoController(
        ...
        @DeleteMapping("/excluir-lote/{ids}")
        fun excluirAtendimentoLote(@PathVariable ids: List<Int>): ResponseEntity<Void> {
            val sucesso = atendimentoService.excluirAtendimentoLote(ids)
            return if (sucesso) {
                ResponseEntity.ok().build()
            } else {
                ResponseEntity.notFound().build()
